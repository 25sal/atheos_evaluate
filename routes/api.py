    ...
    if provider:
        ...
        if user:
            ...
            if(user.is_forbidden):
                return jsonify({"message": "Forbidden to Login"})
            else:
                return jsonify({"access_token": str(access_token.decode())})

    return jsonify({"message": "Invalid credentials"}), 401


@api_bp.route('/forced_logout', methods=['POST'])
@jwt_required()
def forced_logout():
    data = request.form
...
def exec():
    ...
    if os.path.isdir(user_folder):
        ...
        reply['passed'] = logs[1]
        reply['failed'] = logs[2]
        savetestresult(get_userid(current_identity.email),logs[1:3],exId['exercise'])
        for key in checker.checks.keys():
            savechecks(current_identity.email, key, checker.checks[key])
            reply[key] = checker.checks[key]
        return jsonify(reply)
    reply['logs'] = user_folder + exer_folder
    reply['passed'] = 0
    reply['failed'] =  0
    return jsonify(reply)
...
@jwt_required()
def api_results():
    checks = getchecks(current_identity.email)
    if checks is None:
        return json.dumps({'success': False})
   
    results = getresults(get_userid(current_identity.email),getproject(current_identity.email)['exercise'])
    if len(results) > 0:
