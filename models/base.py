...
class Users(UserMixin, db.Model):
    ...
    forced_to_logout = db.Column(db.Integer, default = 0)
    ...
    def is_forbidden(self):
        return self.forced_to_logout == self.FORBIDDEN
...
def saveForbidden(user_id):

    t = text('UPDATE users set forced_to_logout=:val1 where id=:val2')
    db.session.execute(t,{'val1':1,'val2':user_id})
    db.session.commit()
